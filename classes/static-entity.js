class StaticEntity {
	...
	draw() {
		if (this.x < camera.x + camera.cw && this.x + this.width > camera.x &&
			...
			if (this.shadowSprite !== null) this.shadowSprite.draw(this.x + this.shadowOffset.x, this.y + this.shadowOffset.y);

			if (this.isChanging) this.mainSprite.draw(c, this.spriteIDs[this.spriteID],
				this.width, this.height, this.x, this.y);
			else this.mainSprite.draw(this.x, this.y, c);
		}
